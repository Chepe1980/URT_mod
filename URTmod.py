...
def estimate_toc_combined(df, method='passey', baseline_RT=None, baseline_GR=None):
    """Combined TOC estimation"""
    
    if method == 'passey':
        df = estimate_toc_passey(df, baseline_RT=baseline_RT, baseline_GR=baseline_GR)
    elif method == 'schmoker':
        df = estimate_toc_schmoker(df)
    elif method == 'ensemble':
        df_passey = estimate_toc_passey(df.copy(), baseline_RT=baseline_RT, baseline_GR=baseline_GR)
...
def calibrate_toc_with_lab_data(df, lab_data):
    """Calibrate TOC with laboratory measurements"""
...
def load_and_preprocess_data(df, estimate_toc=True, toc_method='passey'):
    ...
    if 'TOC' not in df.columns and estimate_toc:
        df = estimate_toc_combined(df, method=toc_method)
...
def create_toc_plot(df):
    """Create TOC visualization"""
...
def main():
    ...
    if use_sample:
        ...
        sample_data = {
            ...
            'PHIE': np.random.uniform(0.05, 0.15, n_samples),
            'VP': np.random.uniform(3000, 4500, n_samples),
            'VS': np.random.uniform(1500, 2500, n_samples),
            'RHOB': np.random.uniform(2.2, 2.7, n_samples),
            'VSH': np.random.uniform(0.2, 0.6, n_samples),
            'RT': np.random.uniform(10, 100, n_samples),
            'GR': np.random.uniform(50, 150, n_samples),
        }
        
        # Add some trends
        sample_data['PHIE'] = 0.1 + 0.05 * np.sin(np.linspace(0, 4*np.pi, n_samples))
        sample_data['TOC'] = 2 + 3 * np.sin(np.linspace(0, 3*np.pi, n_samples)) + np.random.normal(0, 0.5, n_samples)
        sample_data['TOC'] = sample_data['TOC'].clip(0, 10)
    ...
    if st.session_state.get('data_loaded', False) and 'df_processed' in st.session_state:
        ...
        with tab1:
            st.plotly_chart(create_toc_plot(df), use_container_width=True)
